...
class ReportChartViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let yVals = (0..<10).map { (i) -> BarChartDataEntry in
            let mult = 10 + 1
            let val1 = Double(Int(arc4random_uniform(UInt32(mult))) + mult / 3)
            let val2 = Double(Int(arc4random_uniform(UInt32(mult))) + mult / 3)
            let val3 = Double(Int(arc4random_uniform(UInt32(mult))) + mult / 3)
            
            return BarChartDataEntry(x: Double(i), yValues: [val1, val2, val3])
        }
        let set2 = BarChartDataSet(entries: yVals)
